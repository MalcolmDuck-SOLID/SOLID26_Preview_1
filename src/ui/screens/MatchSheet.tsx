...
import { MatchResult } from '../../match/findMatches';
...
export const MatchSheet: React.FC<MatchSheetProps> = ({ matches, onClose }) => {
  ...
  const handleShare = async () => {
    ...
    setSharing(true);
    try {
      await shareCard(selectedCard, selectedMatch.contact, session.fetch);
      setSuccess(true);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative bg-zinc-900 border-t border-zinc-800 rounded-t-3xl p-6 shadow-[0_-20px_50px_rgba(0,0,0,0.5)] animate-in slide-in-from-bottom pb-10">
        <button onClick={onClose} className="absolute top-4 right-4 text-zinc-500 hover:text-zinc-200">
          <X size={24} />
        </button>
        
        <h3 className="text-2xl font-bold text-zinc-50 mb-6">Found in the Wild</h3>
        ...
        {success ? (
          ...
          <>
            <div className="mb-6">
              <label className="block text-sm font-medium text-zinc-400 mb-3">1. Select Contact</label>
                ...
                {matches.map(m => (
                  <button
                    ...
                    className={`w-full flex flex-col text-left p-3 rounded-xl border transition-colors ${
                      selectedMatch?.contact === m.contact ? 'bg-blue-500/10 border-blue-500' : 'bg-zinc-800 border-zinc-700 hover:border-zinc-500'
                    }`}
                  >
                    <span className="font-medium text-zinc-200 truncate">{m.contact}</span>
                    <span className="text-xs text-zinc-500">Also in {m.cityName}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="mb-8">
              <label className="block text-sm font-medium text-zinc-400 mb-3">2. Select Card to Share</label>
              <div className="flex space-x-3 overflow-x-auto pb-2 scrollbar-hide">
                {cards.length === 0 ? (
                   <div className="text-zinc-500 text-sm">No cards available. Create one first.</div>
                ) : cards.map(c => (
                  <button
                    key={c.url}
                    onClick={() => setSelectedCard(c.url)}
                    className={`shrink-0 px-4 py-2 rounded-full border transition-colors ${
                      selectedCard === c.url ? 'bg-blue-500 text-white border-blue-400' : 'bg-zinc-800 text-zinc-300 border-zinc-700 hover:border-zinc-500'
                    }`}
                  >
                    {c.label}
                  </button>
